...
class ScoresHead(nn.Module):
        ...
        for epoch in range(epochs):
            ...
            if validation:
                val_loss = self.evaluate(
                    val_titles, val_bodies, val_combined, val_labels
                )
            ...
            if verbose:
                ...
                if validation:
                    ...
                    print(
                        "Validation Accuracy: ",
                        accuracy(
                            np.where(outputs.detach().cpu().numpy() > 0.5, 1, 0),
    ...
    def evaluate(self, titles, bodies, combined, labels):
        ...
        outputs = self.forward(
            titles_tensor, bodies_tensor, combined_tensor, train=False
        )
        ...
        if self.use_head:
            ...
            if self.device == "cuda":
                head.load_state_dict(
                    torch.load(os.path.join(self.model_path, "scores_head.pt"))
                )
            else:
                head.load_state_dict(
                    torch.load(
                        os.path.join(self.model_path, "scores_head.pt"),
