...
def get_n_chunks(df, chunk_size):
    ...
    return (
        int(df_size / chunk_size)
        if df_size % chunk_size == 0
        else int(df_size / chunk_size) + 1
    )
...
def df_to_txt(
    df, return_text=False, save=True, path="./", name="text", extension="txt"
):
    """
...
def split_to_classes(
    ...
    questions_df = df.query(
        "label_bug == 0 and \
                             label_question == 1 and \
                             label_enhancement == 0"
    )

    enhancements_df = df.query(
        "label_bug == 0 and \
                                label_question == 0 and \
                                label_enhancement == 1"
    )

    combinations_df = df.query(
        "(label_bug == 1 and label_question == 1 and label_enhancement == 0) or \
...
def sample_df(
    ...
    """
    assert n != frac
    bugs_df, questions_df, enhancements_df, combinations_df = split_to_classes(
        df, to_keep
    )
